...
// providers/perplexity.js - Perplexity provider (heuristic DOM).
// last validated: 2026-07 — perplexity.ai
...
const ZSProvider = (() => {
  ...
  const S = {
  "turn": "[class*=\"Query\"], [class*=\"Answer\"], [data-testid*=\"message\"]",
  "user": "[class*=\"Query\"], [data-testid*=\"user\"]",
  "asst": "[class*=\"Answer\"], [data-testid*=\"assistant\"]",
  "editor": "textarea, div.ProseMirror[contenteditable=\"true\"], [contenteditable=\"true\"]",
  "sendBtn": "button[aria-label*=\"Submit\"], button[aria-label*=\"Send\"]",
  ...
  async function typeAndSend(text, images) {
    const ed = getEditor(); if (!ed) throw new Error("Perplexity input box not found");
  ...
  async function ensureComposerReady(reason) {
    diag("mode_ready", { reason, provider: "perplexity", ready: !!getEditor() });
  ...
  function modeWarning() {
    try {
      if (!getEditor()) return "Perplexity composer not found — open a New Thread or refresh.";
    } catch {}
    return "";
  }
  function unstableWarning() {
    return "Perplexity favors search snippets — remind it to emit tool commands, not just cite sources.";
  }
  return {
    id: "perplexity", displayName: "Perplexity", supportsVision: true,
