...
// providers/t3chat.js - T3 Chat provider (heuristic DOM).
...
const ZSProvider = (() => {
  ...
  async function typeAndSend(text, images) {
    const ed = getEditor(); if (!ed) throw new Error("T3 Chat input box not found");
  ...
  async function ensureComposerReady(reason) {
    diag("mode_ready", { reason, provider: "t3chat", ready: !!getEditor() });
  ...
  return {
    id: "t3chat", displayName: "T3 Chat", supportsVision: true,
