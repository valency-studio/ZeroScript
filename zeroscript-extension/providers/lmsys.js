...
// providers/lmsys.js - LMSYS Chatbot Arena provider (heuristic DOM).
...
const ZSProvider = (() => {
  ...
  const S = {
  "turn": "[class*=\"message\"], .chat-message, [data-testid*=\"message\"]",
  "user": "[class*=\"user\"], .user-message",
  "asst": "[class*=\"bot\"], .bot-message, [class*=\"assistant\"]",
  "editor": "textarea, [contenteditable=\"true\"]",
  "sendBtn": "button[aria-label*=\"Send\"], button#component-submit, button[type=\"submit\"]",
  ...
  async function typeAndSend(text, images) {
    const ed = getEditor(); if (!ed) throw new Error("LMSYS Chatbot Arena input box not found");
  ...
  async function ensureComposerReady(reason) {
    diag("mode_ready", { reason, provider: "lmsys", ready: !!getEditor() });
  ...
  return {
    id: "lmsys", displayName: "LMSYS Chatbot Arena", supportsVision: true,
