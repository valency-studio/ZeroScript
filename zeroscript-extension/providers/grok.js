...
// providers/grok.js - Grok provider (heuristic DOM).
// last validated: 2026-07 — grok.com
...
const ZSProvider = (() => {
  ...
  const S = {
  "turn": "[data-testid=\"message\"], [class*=\"message-bubble\"], article",
  "user": "[data-testid=\"user-message\"], [data-message-author-role=\"user\"]",
  "asst": "[data-testid=\"assistant-message\"], [data-message-author-role=\"assistant\"]",
  "editor": "div.ProseMirror[contenteditable=\"true\"], textarea, [contenteditable=\"true\"]",
  ...
  async function typeAndSend(text, images) {
    const ed = getEditor(); if (!ed) throw new Error("Grok input box not found");
  ...
  async function ensureComposerReady(reason) {
    diag("mode_ready", { reason, provider: "grok", ready: !!getEditor() });
  ...
  function modeWarning() {
    try {
      if (!getEditor()) return "Grok composer not found — open a New chat or refresh.";
    } catch {}
    return "";
  }
  function unstableWarning() {
    return "Heuristic DOM — if turn detection looks wrong, reload grok.com and start a fresh chat.";
  }
  return {
    id: "grok", displayName: "Grok", supportsVision: true,
