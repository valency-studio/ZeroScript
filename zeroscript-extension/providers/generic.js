...
// providers/generic.js - Generic AI provider (heuristic DOM).
// last validated: 2026-07 — generic (heuristic)
...
const ZSProvider = (() => {
  ...
  const S = {
  "turn": "[data-message-author-role], [data-testid*=\"message\"], [class*=\"chat-message\"], [class*=\"ChatMessage\"]",
  "user": "[data-message-author-role=\"user\"], [data-testid*=\"user-message\"]",
  "asst": "[data-message-author-role=\"assistant\"], [data-testid*=\"assistant-message\"]",
  "editor": "div.ProseMirror[contenteditable=\"true\"], [contenteditable=\"true\"], textarea",
  "sendBtn": "button[data-testid*=\"send\"], button[aria-label*=\"Send\"], button[type=\"submit\"]",
  "stopBtn": "button[data-testid*=\"stop\"], button[aria-label*=\"Stop\"]"
};
  const RE = {
  ...
  function modeWarning() {
    try {
      if (!getEditor()) return "No chat composer detected on this page — navigate to the site's AI chat or refresh.";
    } catch {}
    return "";
  }
  function unstableWarning() {
    return "Experimental: DOM heuristics may miss the composer or message list — prefer ChatGPT or Claude when possible.";
  }
  return {
    id: "generic", displayName: "Generic AI", supportsVision: true,
