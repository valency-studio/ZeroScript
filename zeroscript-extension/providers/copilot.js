...
// providers/copilot.js - Copilot provider (heuristic DOM).
// last validated: 2026-07 — copilot.microsoft.com
...
const ZSProvider = (() => {
  ...
  const S = {
  "turn": "[data-content=\"user-message\"], [data-content=\"ai-message\"], [class*=\"chat-message\"]",
  "user": "[data-content=\"user-message\"], [data-author=\"user\"]",
  "asst": "[data-content=\"ai-message\"], [data-author=\"bot\"]",
  "editor": "textarea[data-testid=\"composer-input\"], textarea#userInput, [contenteditable=\"true\"]",
  "sendBtn": "button[data-testid=\"submit-button\"], button[aria-label*=\"Send\"]",
  "stopBtn": "button[aria-label*=\"Stop\"], button[aria-label*=\"Cancel\"]"
};
  const RE = {
  ...
  async function typeAndSend(text, images) {
    const ed = getEditor(); if (!ed) throw new Error("Copilot input box not found");
  ...
  async function ensureComposerReady(reason) {
    diag("mode_ready", { reason, provider: "copilot", ready: !!getEditor() });
  ...
  function modeWarning() {
    try {
      if (!getEditor()) return "Copilot composer not found — open a New chat or refresh.";
    } catch {}
    return "";
  }
  function unstableWarning() {
    return "Copilot's web UI changes often — if chips miss replies, reload and use a blank conversation.";
  }
  return {
    id: "copilot", displayName: "Copilot", supportsVision: true,
