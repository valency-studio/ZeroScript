...
// providers/clickup.js - ClickUp AI (app.clickup.com) — prefer Brain2 / Super Agents.
// last validated: 2026-07 — heuristic DOM (ClickUp AI panel evolves often).
// Same ZSProvider interface as providers/deepseek.js / generic.js.
const ZSProvider = (() => {
  "use strict";
  ...
  const S = {
  "turn": "[data-test=\"message-item\"], [class*=\"message-item\"], [class*=\"MessageItem\"], [data-message-author-role], [class*=\"chat-message\"], [class*=\"ChatMessage\"], [class*=\"ai-message\"], [class*=\"AiMessage\"]",
  "user": "[data-test*=\"user-message\"], [data-message-author-role=\"user\"], [class*=\"user-message\"], [class*=\"UserMessage\"]",
  "asst": "[data-test*=\"assistant-message\"], [data-message-author-role=\"assistant\"], [class*=\"assistant-message\"], [class*=\"AiMessage\"], [class*=\"bot-message\"]",
  "editor": "div.ProseMirror[contenteditable=\"true\"], [contenteditable=\"true\"][role=\"textbox\"], [contenteditable=\"true\"], textarea[placeholder*=\"Ask\"], textarea[placeholder*=\"Message\"], textarea",
  "sendBtn": "button[data-test*=\"send\"], button[aria-label*=\"Send\"], button[type=\"submit\"], button[class*=\"send\"]",
  "stopBtn": "button[data-test*=\"stop\"], button[aria-label*=\"Stop\"], button[aria-label*=\"Cancel\"]"
};
  const RE = {
    ...
    busy: /something went wrong|try again later|rate limit|too many requests|une erreur|overloaded|daily limit/i,
  };
  ...
  async function typeAndSend(text, images) {
    const ed = getEditor(); if (!ed) throw new Error("Generic AI input box not found");
  ...
  async function ensureComposerReady(reason) {
    diag("mode_ready", { reason, provider: "generic", ready: !!getEditor() });
  ...
  function modeWarning() {
    try {
      if (!getEditor()) return "Open ClickUp AI chat (Brain / Super Agents). Prefer Brain2 when available.";
    } catch {}
    return "";
  }
  function unstableWarning() {
    return "ClickUp: open AI chat and select Brain2 (multi-model). Daily limits apply. If the bar misses the composer, refresh or open a blank AI thread.";
  }
  return {
    id: "clickup", displayName: "ClickUp", supportsVision: true,
