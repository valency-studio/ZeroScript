...
// providers/poe.js - Poe provider (heuristic DOM).
// last validated: 2026-07 — poe.com
...
const ZSProvider = (() => {
  ...
  const S = {
  "turn": "[class*=\"Message\"], [data-testid*=\"message\"], [class*=\"ChatMessage\"]",
  "user": "[class*=\"ChatMessage_right\"], [data-testid*=\"user\"]",
  "asst": "[class*=\"ChatMessage_left\"], [class*=\"BotMessage\"], [data-testid*=\"bot\"]",
  "editor": "textarea, div.ProseMirror[contenteditable=\"true\"], [contenteditable=\"true\"]",
  "sendBtn": "button[class*=\"send\"], button[aria-label*=\"Send\"]",
  ...
  async function typeAndSend(text, images) {
    const ed = getEditor(); if (!ed) throw new Error("Poe input box not found");
  ...
  async function ensureComposerReady(reason) {
    diag("mode_ready", { reason, provider: "poe", ready: !!getEditor() });
  ...
  function modeWarning() {
    try {
      if (!getEditor()) return "Poe composer not found — open a bot chat or refresh.";
    } catch {}
    return "";
  }
  function unstableWarning() {
    return "Pick a coding-capable bot and keep one bot per session — switching bots mid-thread can confuse turn detection.";
  }
  return {
    id: "poe", displayName: "Poe", supportsVision: true,
