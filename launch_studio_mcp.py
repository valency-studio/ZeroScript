...
def _candidate_roots() -> list[Path]:
    ...
    if local_appdata:
        roots.append(Path(local_appdata) / "Roblox" / "Versions")      # ← official
        roots.append(Path(local_appdata) / "Bloxstrap" / "Versions")   # ← bloxstrap
        roots.append(Path(local_appdata) / "Fishstrap" / "Versions")   # ← fishstrap
    for env in ("ProgramFiles", "ProgramFiles(x86)"):
        value = os.environ.get(env)
        if value:
            roots.append(Path(value) / "Fishstrap" / "Versions")
            roots.append(Path(value) / "Bloxstrap" / "Versions")
    return roots
